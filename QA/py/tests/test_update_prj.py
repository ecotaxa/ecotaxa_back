...
PROJECT_SETTINGS_UPDATE_URL = (
    "/projects/{project_id}/prediction_settings?settings={settings}"
)
...
def test_update_prj_pred_settings(database, fastapi, caplog):
    ...
    sett_url = PROJECT_SETTINGS_UPDATE_URL.format(
        project_id=prj_id, settings=quote(new_settings)
    )
