...
class SamplesService(Service):
    ...
    def query(
        self, current_user_id: Optional[UserIDT], sample_id: SampleIDT
    ) -> Optional[SampleBO]:
        ...
        if current_user_id is None:
            project = RightsBO.anonymous_wants(
                self.ro_session, Action.READ, ret.sample.projid
            )
        else:
            _user, project = RightsBO.user_wants(
                self.session, current_user_id, Action.READ, ret.sample.projid
            )
...
class AcquisitionsService(Service):
    ...
    def query(
        self, current_user_id: Optional[int], acquisition_id: AcquisitionIDT
    ) -> Optional[AcquisitionBO]:
        ...
        if current_user_id is None:
            project = RightsBO.anonymous_wants(
                self.ro_session, Action.READ, ret.acquis.sample.projid
            )
        else:
            _user, project = RightsBO.user_wants(
                self.session, current_user_id, Action.READ, ret.acquis.sample.projid
            )
        mappings = ProjectMapping().load_from_project(project)
    ...
    def update_set(
        self,
        current_user_id: int,
        acquisition_ids: AcquisitionIDListT,
        updates: ColUpdateList,
    ) -> int:
        ...
        assert len(prj_ids) == 1, (
            "Too many or no projects for acquisitions: %s" % acquisition_ids
        )
        prj_id = prj_ids[0]
        _user, project = RightsBO.user_wants(
            self.session, current_user_id, Action.ADMINISTRATE, prj_id
...
class ProcessesService(Service):
    ...
    def query(
        self, current_user_id: Optional[int], process_id: ProcessIDT
    ) -> Optional[ProcessBO]:
        ...
        if current_user_id is None:
            project = RightsBO.anonymous_wants(
                self.ro_session, Action.READ, ret.process.acquisition.sample.projid
            )
        else:
            _user, project = RightsBO.user_wants(
                self.session,
    ...
    def update_set(
        self, current_user_id: int, process_ids: ProcessIDListT, updates: ColUpdateList
    ) -> int:
        ...
        # All should be in same project, so far
        assert len(prj_ids) == 1, (
            "Too many or no projects for processes: %s" % process_ids
        )
        prj_id = prj_ids[0]
        _user, project = RightsBO.user_wants(
            self.session, current_user_id, Action.ADMINISTRATE, prj_id
