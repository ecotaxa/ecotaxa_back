...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "lastserverversioncheck_datetime",
    ...
    op.add_column(
        "taxonomy",
        sa.Column(
            "creation_datetime", postgresql.TIMESTAMP(precision=0), nullable=True
        ),
    )
    op.add_column(
        "taxonomy", sa.Column("creator_email", sa.VARCHAR(length=255), nullable=True)
    )
    op.add_column(
        "taxonomy", sa.Column("display_name", sa.VARCHAR(length=200), nullable=True)
    )
    op.add_column("taxonomy", sa.Column("id_instance", sa.INTEGER(), nullable=True))
    op.add_column(
        "taxonomy",
        sa.Column(
            "lastupdate_datetime", postgresql.TIMESTAMP(precision=0), nullable=True
        ),
    )
    op.add_column("taxonomy", sa.Column("rename_to", sa.INTEGER(), nullable=True))
    op.add_column(
        "taxonomy", sa.Column("source_desc", sa.VARCHAR(length=1000), nullable=True)
    )
    op.add_column(
        "taxonomy", sa.Column("source_url", sa.VARCHAR(length=200), nullable=True)
    )
    op.add_column(
        "taxonomy",
        sa.Column("taxostatus", sa.CHAR(length=1), server_default="A", nullable=False),
    )
    op.add_column(
        "taxonomy",
        sa.Column("taxotype", sa.CHAR(length=1), server_default="P", nullable=False),
    )
    # ### end Alembic commands ###
    op.execute(
        """UPDATE taxonomy set taxotype='M'
...
'temporary','transparent','tubes','tuff','turbid','unicellular','veliger','Viruses','wing','zoea')
   """
    ...
    op.execute(
        """update public.taxonomy set lastupdate_datetime ='2018-01-02',display_name=name """
    )
    op.execute("create index IS_TaxonomyDispNameLow on taxonomy(lower(display_name))")
    op.execute(
        """insert into persistantdatatable(id,lastserverversioncheck_datetime) VALUES (1,'2018-01-02')"""
    )
