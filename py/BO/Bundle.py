...
class InBundle(object):
    ...
    def __init__(self, path: str, user_loc: str, temp_dir: Path):
        ...
        def one_more() -> None:
            ...
            if nb_files > self.MAX_FILES:
                raise ImportError(
                    "You tried to import too many files, max. is %d" % self.MAX_FILES
                )
    ...
    def do_import(
        ...
        how.existing_samples, how.existing_acquisitions = self.fetch_existing_parents(
            session, prj_id=how.prj_id
        )
        logger.info("existing samples = %s", list(how.existing_samples.keys()))
        logger.info(
            "existing acquisitions = %s", list(how.existing_acquisitions.keys())
        )
    ...
    def import_each_file(
        ...
        def log_stats(name: str, rows: int) -> None:
            ...
            logger.info(
                "File %s : %d rows loaded, %d so far at %d rows/s",
                name,
                rows,
                stats.current_row_count,
                rows_per_sec,
            )
    ...
    def fetch_existing_parents(
        ...
        """
        ret = (
            Sample.get_orig_id_and_model(session, prj_id),
    ...
    def validate_import(
        ...
        if total_row_count == 0:
            ...
            if nb_found == 0:
                err_msg.append(
                    "* No .txt or .tsv file was found, of which name starts with 'ecotaxa'."
                )
            else:
                ...
                if nb_skipped > 0:
                    if nb_validated == 0:
                        err_msg.append(
                            "* 'SKIP TSV' option was set and all TSV files were imported before."
                        )
                    else:
                        err_msg.append(
                            "* 'SKIP TSV' option was set and new TSV file(s) are not compliant."
                        )
                if nb_validated > 0:
                    err_msg.append("*  TSV file(s) might be empty.")
                if how.skip_object_duplicates:
                    err_msg.append(
                        "*  'SKIP OBJECTS' option was set and all objects might be in already."
                    )
        ...
        if len(not_seen_fields) > 0:
            diag.warn(
                "Some fields configured in the project are not seen in this import {0} ".format(
                    ", ".join(not_seen_fields)
                )
            )
        if diag.nb_objects_without_gps > 0:
            diag.warn(
                "{0} object(s) don't have GPS information.".format(
                    diag.nb_objects_without_gps
                )
            )
...
class UVP6Bundle(InBundle):
    ...
    def before_import(self, how: ImportHow) -> None:
        ...
        if potential_config.exists():
            ...
            how.vignette_maker = VignetteMaker(
                vignette_maker_cfg, self.path, self.TEMP_VIGNETTE
            )
