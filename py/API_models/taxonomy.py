...
class TaxonModel(BaseModel):
    ...
    type: str = Field(title="Type", description="The taxon/category type, 'M' for Morpho or 'P' for Phylo.",
                      example="P")
    nb_objects: int = Field(title="Nb objects", description="How many objects are classified in this category.",
                            example=34118)
    nb_children_objects: int = Field(title="Nb children objects",
    ...
    lineage: List[str] = Field(title="Lineage",
                               description="The taxon/category name of ancestors, including self, in first.",
                               example=["living"])
    id_lineage: List[int] = Field(title="Id lineage",
                                  description="The taxon/category IDs of ancestors, including self, in first.",
                                  example=[1])
    children: List[int] = Field(title="Children", description="The taxon/category IDs of children.",
                                example=[92952, 2, 92329, 85048, 4, 93599, 93687, 85011, 92951, 93698, 84961, 92696, 3])


class TaxonUsageModel(ProjectSummaryModel):
    nb_validated: int = Field(title="Nb validated",
                              description="How many validated objects in this category in this project.", example=129)


class TaxonomyTreeStatus(BaseModel):
    last_refresh: Optional[str] = Field(title="Last refresh",
