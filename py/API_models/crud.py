...
class _MinUserModel(DescriptiveModel):
    ...
    name = Field(title="Name", description="User's full name, as text.", example="userName")
...
class _FullUserModel(_MinUserModel):
    organisation = Field(title="Organisation", description="User's organisation name, as text.",
                         example="Oceanographic Laboratory of Villefranche sur Mer - LOV")
    active = Field(title="Account status", description="Whether the user is still active.",
                   example=True)
...
class ProjectSummaryModel(BaseModel):
    projid: int = Field(title="Project Id", description="Project unique identifier.", example=1)
    title: str = Field(title="Project title", description="Project's title.", example="Zooscan Tara Med")
...
class _Acquisition2Model(DescriptiveModel):
    acquisid = Field(title="Acquisition Id", description="The acquisition Id.", example=144)
    ...
    instrument = Field(title="Instrument", description="Instrument used.", example="uvp5")
...
class _Collection2Model(DescriptiveModel):
    ...
    external_id_system = Field(title="External id system", description="The external Id system.",
                               example="")
    title = Field(title="Title", description="The collection title.", example="My collection")
    short_title = Field(title="Short title", description="The collection short title.",
                        example="My coll")
    citation = Field(title="Citation", description="The collection citation.", example="")
    license = Field(title="License", description="The collection license.",
                    example=LicenseEnum.CC_BY)
    abstract = Field(title="Abstract", description="The collection abstract.", example="")
    description = Field(title="Description", description="The collection description.", example="")
...
class _DBSampleTaxoStatsDescription(DescriptiveModel):
    ...
    nb_validated = Field(title="Number validated",
    ...
    nb_predicted = Field(title="Number predicted",
                         description="The number of predicted objects inside the sample.")


SampleTaxoStatsModel = dataclass_to_model_with_suffix(SampleTaxoStats, _DBSampleTaxoStatsDescription)
...
class ProcessModel(_ProcessModelFromDB):
    free_columns: Dict[str, Any] = Field(title="Free columns from process mapping in project",
                                         default={},
                                         example={"software": "zooprocess_pid_to_ecotaxa_7.26_2017/12/19",
                                                  "pressure_gain": "10"})


class CreateProjectReq(BaseModel):
    clone_of_id: Optional[int] = Field(title="Clone of id",
...
class BulkUpdateReq(BaseModel):
    target_ids: List[int] = Field(title="Target Id", description="The IDs of the target entities.",
                                  example=[1, 5, 290])
    updates: List[ColUpdate] = Field(title="Updates", description="The list of updates, to do on all impacted entities. \n\n \
    { \n\n \
...
class ProjectTaxoStatsModel(BaseModel):
    ...
    nb_validated: int = Field(title="nb_validated",
    ...
    nb_dubious: int = Field(title="nb_dubious", description="The number of dubious objects inside the project.",
                            example=56)
    nb_predicted: int = Field(title="nb_predicted",
...
ProjectUserStatsModel = dataclass_to_model_with_suffix(ProjectUserStats, _DBProjectUserStatsDescription)
...
ProjectSetColumnStatsModel = dataclass_to_model_with_suffix(ProjectSetColumnStats, _DBProjectSetColumnStatDescription)
...
class _Job2Model(DescriptiveModel):
    ...
    progress_msg = Field(title="progress_msg", description="The message for UI, short version. ",
                         example="Done")
...
class _AddedToJob(BaseModel):
    ...
    result: Dict[str, Any] = Field(title="result", description="Final result of the run.", default={},
                                   example={"rowcount": 3})
    errors: List[str] = Field(title="errors", description="The errors seen during last step.", default=[],
                              example=[])
