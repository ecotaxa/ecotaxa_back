...
def upgrade():
    ...
    tbl = op.create_table('instrument',
    ...
    instrs = [{'instrument_id': ins, 'name': nam, 'bodc_url': url}
              for ins, nam, url in DEFAULT_INSTRUMENTS]
    op.bulk_insert(tbl, instrs)
    default = DEFAULT_INSTRUMENTS[-1][0]
    op.add_column('projects', sa.Column('instrument_id', sa.VARCHAR(length=32), nullable=False, server_default=default))
    op.alter_column('projects', 'instrument_id', server_default=None)
    op.create_foreign_key(None, 'projects', 'instrument', ['instrument_id'], ['instrument_id'])
